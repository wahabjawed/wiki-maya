"""
This module provides a :class:`maya.api.Session` class that can maintain a
client connection to an instance of maya and efficiently generate scores.

Batching and parallelism are set by constructor arguments.

.. autoclass:: maya.api.Session
    :members:
"""
import logging
import time
...
class Session:
    """
    Constructs a session with an maya API and provides facilities for scoring
    revisions in batch and parallel.

    :Parameters:
        host : str
            The host of maya to connect to (preceed with http:// or https://)
        user_agent : str
        ...
        retries : int
            ...
            up
    """
    DEFAULT_USERAGENT = "maya.api default user-agent"
        ...
        with ThreadPoolExecutor(max_workers=self.workers) as executor:
            ...
            for (future, rev_id_batch) in future_rev_ids:
                try:
                    ...
                    logger.warning(
                        "An maya scoring job failed with the following error:")
    ...
    def _score_request(self, context, rev_ids, models):
        url = self.host + "/v1/scores/{0}/".format(urllib.parse.quote(context))
