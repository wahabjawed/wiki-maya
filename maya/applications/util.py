...
def build_config(config_dirs=DEFAULT_DIRS, **kwargs):
    ...
    if 'data_paths' in config['maya'] and \
       'nltk' in config['maya']['data_paths']:
        import nltk
        nltk.data.path.append(config['maya']['data_paths']['nltk'])
...
def configure_logging(verbose=False, debug=False, config=None, **kwargs):
    ...
    if debug:
        ...
        logging.getLogger('maya.metrics_collectors.logger') \
