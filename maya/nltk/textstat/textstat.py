...
class textstatistics:
    ...
    def setup(self, text):
        self.v_char_count = self.char_count(text)
        self.v_letter_count = self.letter_count(text)
        self.v_remove_punctuation = self.remove_punctuation(text)
        self.v_lexicon_count = self.lexicon_count(text)
        self.v_sentence_count = self.sentence_count(text)
        self.v_avg_sentence_length = self.avg_sentence_length(text)

    def compute(self):
        return [self.flesch_kincaid_grade(self.text), self.smog_index(self.text),
                self.coleman_liau_index(self.text), self.automated_readability_index(self.text),
                self.linsear_write_formula(self.text), self.dale_chall_readability_score(self.text),
    ...
    def char_count(self, text, ignore_spaces=True):
        ...
        """
        if self.v_char_count > 0:
            return self.v_char_count
        else:
            if ignore_spaces:
                text = text.replace(" ", "")
            return len(text)
    ...
    def letter_count(self, text, ignore_spaces=True):
        ...
        to ignore whitespaces
        """
        if self.v_letter_count > 0:
            return self.v_letter_count
        else:
            if ignore_spaces:
                text = text.replace(" ", "")
    ...
    def __get_lang_easy_words(self):
        try:
            ...
            path = os.path.join(my_path, "resources/en/easy_words.txt")
