...
from ..nlp_request import NLPRequest
...
def build_nlp_request(request, context_name=None, rev_id=None,
                        ):
    """
    Build an :class:`maya.ScoreRequest` from information contained in a
    request.

    :Parameters:
        scoring_system : :class:`maya.ScoringSystem`
    ...
    return NLPRequest(context_name, rev_ids,
                        ip=ip)
...
def build_event_set(event):
    """
    Turn an EventStream event into a set of event types that maya
    uses internally.
    """
    event_set = set()
...
def build_precache_map(config):
    ...
    """
    precache_map = {}
    ss_name = config['maya']['scoring_system']
